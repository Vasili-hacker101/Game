    ...
    def update(self, run, left, right, up, platforms, monsters):
        ...
        if up:
            if self.onGround and self.y_speed == 0:
                ...
                ticks = pygame.time.get_ticks() - jump_time
                if ticks > 150:
                    self.y_speed = self.y_speed - hero.jump_mult
                    hero.jump_mult = 0
    ...
    while running:
        ...
        if hero.dead:
            ...
            if hero.hp > 1:
                ...
                game_over_str = u"       Вы проиграли! Ваш счет: " + str(hero.num_of_coins)
                message.blit(message_font.render(game_over_str, 1, (255, 255, 255)), (10, 5))
                screen.blit(message, (0, 300))
                pygame.display.flip()
                pygame.time.wait(4000)
        ...
        for e in pygame.event.get():
            ...
            if e.type == pygame.KEYDOWN and (e.key == pygame.K_UP or e.key == pygame.K_SPACE):
                up = True
                jump_time = pygame.time.get_ticks()
                hero.jump_mult = JUMP_POWER
            ...
            if e.type == pygame.KEYUP and (e.key == pygame.K_UP or e.key == pygame.K_SPACE):
                up = False
